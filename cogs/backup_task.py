...
# --- 設定用於發送備份的 Discord 頻道 ID ---
# 🌟🌟🌟 非常重要：請將 YOUR_BACKUP_CHANNEL_ID_HERE 替換為你想接收備份的 Discord 頻道 ID 🌟🌟🌟
#
# 如何獲取頻道 ID：
# 1. 在 Discord 桌面應用程式中，前往「使用者設定」（左下角的齒輪圖示）。
# 2. 點擊「進階」（Advanced）。
# 3. 開啟「開發者模式」（Developer Mode）。
# 4. 關閉使用者設定，回到 Discord 主介面。
# 5. 在你想要作為備份目標的 Discord 頻道上，右鍵點擊該頻道名稱。
# 6. 選擇「複製 ID」（Copy ID）。
# 7. 將複製的數字貼到下方等號的後面。
