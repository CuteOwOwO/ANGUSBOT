           ...
           current_user_status_info.get("message_id") == reaction.message.id:
            ...
            if str(reaction.emoji) in self.EMOJI_NUMBERS:
                ...
                if chosen_folders_order and 0 <= chosen_index < len(chosen_folders_order):
                    ...
                    self.bot.user_status[user_id]["selected_folder_number"] = selected_folder_number # 儲存選擇的資料夾編號
                    self.bot.user_status[user_id]["selected_folder_name"] = display_name
                    self.bot.user_status[user_id]["message_id"] = reaction.message.id # 儲存訊息ID以便後續操作
                    self.bot.user_status[user_id]["message_channel_id"] = reaction.message.channel.id
